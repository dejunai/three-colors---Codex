...
**Gemini (Drive-embedded)** — an occasional second check on dialogue-gating logic, on hand by virtue of being built into Drive rather than specially invited; its first pass (2026-09-22) independently confirmed the same gate structure DeepSeek's review and the six-fix pass established, no discrepancies found.
...
- **Naming rule.** The trilogy sits next to Lovecraft's lore, not inside it. The only borrowed proper nouns are **Pickman** (the street) and **Miskatonic** (the county and its institutions). No other Lovecraft proper noun (place, deity, book, family or creature) may be added to source, content files, tests or docs. See Design Bible v18, Part Two, Widow's Bight.
