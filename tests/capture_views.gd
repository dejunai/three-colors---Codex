...
func run(g:Node) -> void:
	...
	if g.capture_mode == "gate":
		...
		g._update_camera(1)
	if g.capture_mode in ["town","town_business","town_upper","town_lower","town_waterfront","town_waterfront_seaward","town_waterfront_workyard","precinct","boardinghouse","room","board","witness"]:
		...
		if g.capture_mode == "town_business":
		...
		if g.capture_mode.begins_with("town_"):
			g.pitch = 0.42
			g.distance = 7.2
			if g.capture_mode in ["town_waterfront","town_waterfront_seaward","town_waterfront_workyard"]:
				g.pitch = 0.28
				g.distance = 9.0
			g._update_camera(1.0)
