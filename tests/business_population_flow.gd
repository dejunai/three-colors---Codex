...
const RESIDENTS = ["business_resident_01", "business_resident_02", "business_resident_03", "business_resident_04", "business_resident_05", "business_resident_06"]
...
func run() -> void:
	...
	for sample in [DayClock.MORNING, DayClock.NOON, DayClock.EVENING]:
		...
		for resident in RESIDENTS:
			...
			assert(slot.size() == 2 and slot[0] == "business", resident + " missing from business district")
			var key = str(slot[1])
	...
	print("BUSINESS POPULATION PASS: ", RESIDENTS.size(), " ambient residents, distinct daytime placements: ", seen_by_phase)
	quit()
