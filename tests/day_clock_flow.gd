...
func run() -> void:
	...
	g._interact("boy")
	cards(g)
	assert(g.state.clock_minutes==before+4,"An unpriced default greeting is free")
	g._cards(g.Story.SCENES.boy,g._close)
	cards(g)
	assert(g.state.clock_minutes==before+4,"Exact same substantive dialogue cannot be charged twice")
	g._cards([["A RESIDENT","I've got nothing to say about it."]],g._close)
	cards(g)
	assert(g.state.clock_minutes==before+4)
	g._travel("town",Vector3(0,0.1,8))
	assert(g.state.clock_minutes==before+34)
	g._travel("boardinghouse",Vector3(0,0.1,6))
	assert(g.state.clock_minutes==before+34)
	g._travel("town",Vector3(0,0.1,8))
	g._travel("business",Vector3(10,0.1,25))
	assert(g.state.clock_minutes==before+64)
	g._travel("schoolhouse",Vector3(0,0.1,6))
	g._travel("business",Vector3(-19,0.1,-3))
	assert(g.state.clock_minutes==before+64)
	g._save_game()
	g._load_game()
	assert(g.state.clock_minutes==before+64 and g.state.timed_conversations.has("boy"))
	g._journal()
	g.tick_world(600)
	assert(g.state.clock_minutes==before+64,"Notebook reading is free")
	g._close()
