...
func run() -> void:
	...
	for send_county in [false,true]:
		...
		assert(g._save_game());g._load_game();drain(g)
		assert(g.state.evidence.has("gazette_correction_printed"))
		g._close()
		assert(g.state.clock_minutes==time_before+10)
		assert(g.state.report_evidence==original_report)
		assert(g.state.county_evidence==original_county)
		assert(g.state.supplement_history==reports_before)
		assert(g.state.links.is_empty(),"Printing requires no manual linking")
		assert(not available(g.state,"print_correction"))
		assert(not available(g.state,"the_omitted_two"))
		g.scripted_dialogue.play_topic(g,"gazette_editor","print_correction")
		assert(g.state.clock_minutes==time_before+10)
		...
		# Returning to the newspaper office after showing to Almy must provide valid dialogue (no EOF error).
		g._travel("business",Vector3(3,0.1,16),0,false)
		assert(g.scripted_dialogue.interact(g,"gazette_editor"))
		drain(g);g._close()
		assert(not g.dialogue.cards.is_empty() and g.dialogue.cards[0][0] != "[DEVELOPER WARNING]")
