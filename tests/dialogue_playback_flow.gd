...
''')
	...
	assert(R.menu(decision, R.make_context(S, D)).entries.size() == 1, "A committed outcome must keep its source fork available for review")
	var replay = R.render("choice_test", decision.topics[0], D)
	assert(replay.fork.options == ["Press him."], "Replay must expose only the previously committed branch")
	...
	print("DIALOGUE PLAYBACK PASS: distinct gardener notes, deferred effects/completion, state round-trip, forks, nested continuation, immutable outcomes and locked replay")
	quit(0)
