...
func run() -> void:
	...
	g._travel("post_office", Vector3(0,0.1,6), 0, false)
	var postal_clerk := g.scripted_dialogue.figures.get("post_office_clerk") as Node3D
	assert(is_instance_valid(postal_clerk) and postal_clerk.position.is_equal_approx(Vector3(4,0,-3.05)), "Postal clerk must stand behind the service counter")
	assert(g.estate.points.post_office_clerk.pos.is_equal_approx(Vector3(4,0,-1.55)), "Postal talk point stays clear of the public side of the enlarged counter")
	...
	print("STAFF MODEL INTEGRATION PASS: rendered intake/postal clerks, morgue coroner, and groundskeeper retain accessible placement and Observer tell")
	quit(0)
