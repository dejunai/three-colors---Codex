...
func _run() -> void:
	...
	_play(Runtime.enter(defs.old_woman, ctx, dstate), dstate)
	...
	_play(Runtime.play_topic(defs.almy, dstate, "identify"), dstate)
	...
	_play(Runtime.play_topic(defs.almy, dstate, "almy_trust"), dstate)
	...
	_play(Runtime.play_topic(defs.father_behan, dstate, "club_invitation"), dstate)
