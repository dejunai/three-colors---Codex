...
func _run() -> void:
	...
	if not state.evidence.has("example_door_account"): state.evidence.append("example_door_account")
	assert(Runtime.menu(def, ctx).entries.any(func(entry): return entry.id == "example_decision"), "Committed OUTCOME must leave its source topic available for review")
	var replay_clock = state.clock_minutes
	var replay = Runtime.play_topic(def, ds, "example_decision")
	Runtime.commit_through(replay, state, ds, replay.cards.size())
	assert(replay.fork.options == ["Press for a name."], "Replay must expose only the previously committed branch")
	replay = Runtime.resume(replay, 0)
	Runtime.commit_through(replay, state, ds, replay.cards.size())
	assert(ds.outcome_is("example_account_response", "pressed"), "Replay must not alter the immutable outcome")
	assert(is_equal_approx(state.clock_minutes, replay_clock), "Replaying a resolved decision must not charge time again")
