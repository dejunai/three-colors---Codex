...
`VOICE` is a wordless imitation-of-speech performance, not dialogue content or evidence. It does not persist in case state. Put it directly before the spoken line it accompanies; a dangling cue or a cue immediately before `FORK:` is an authoring error. Inside a fork choice, place it in that choice's indented continuation directly before the reply. Unmarked lines stay silent. The syntax can accompany any spoken card, including `CHOICE`, although the current estate audition deliberately cues NPC lines only. The sound stops when the player advances. Players can reduce or mute these cues independently under **Instrument voices** in Accessibility & controls.
