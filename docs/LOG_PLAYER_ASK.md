...
- **Endpoint hosting: Cloudflare Worker + D1 + R2.** D1 stores allowlisted event rows for SQL aggregation; R2 stores each accepted batch as `events/YYYY-MM-DD/<uuid>.json` for raw verification. Both writes must succeed before the Worker returns HTTP 200 with a small JSON acknowledgement body. Godot 4.7 Web requires that non-empty acknowledgement to retire the delivered client batch; HTTP 204 is observed as a connection error with response code 0. The Worker accepts strict JSON bodies as `application/json` and the same validated schema as `text/plain` solely for Web `sendBeacon` final delivery.
