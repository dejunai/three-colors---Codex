...
Written by Copilot, 2026-09-14, as the sibling system to `docs/DIALOGUE_AUTHORING.md`. This documents the flat-file OBJECT format and its game-facing runtime for world hotspots — examine points and carryable items — as a deliberate parallel to the dialogue system, not a subclass of it. Updated the same day once `estate.object` and `town.object` went live: `chapter_one_objects.gd` (the adapter, mirroring `chapter_one_dialogue.gd`) is wired into `chapter_one.gd`'s `_interact()`/`_town_interaction()`/`_town_observation()` and into every `estate.sync_staging(state)` call site. `tunnel.object` does not exist yet — see `docs/OBJECT_MIGRATION_HOWTO.md` for what's migrated and what's intentionally left alone. The design bible remains authoritative for story content.
...
non-dialogue card sequence. An object's own completion (and its TAG's) is recorded
only after the chosen branch and final continuation finish; a cancelled interaction
earns nothing. A completed object remains available if its `GATE` stays true — for
a once-only state, gate the *next* block on `NOT object_done(location, this_id)`
(see the template's `example_ledger` pair).
