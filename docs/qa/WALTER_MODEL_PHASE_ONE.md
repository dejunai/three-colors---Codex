...
### Independent plain-clothes model — 2026-09-23

The texture-derived plain variant above proved visually false: it recolored the complete police mesh brown, leaving the police coat silhouette, cap, duty belt, holster, pouches, and pockets intact. The badge was the only genuinely separate piece of geometry. It has therefore been retired from runtime use rather than presented as a completed wardrobe switch.

`Meshy_Walter-Plain-Clothes-Animations.glb` supplies a separately modeled Walter in a knee-length civilian wool coat and fedora, with no badge or police equipment. Its Mixamo bone and animation vocabulary matches police Walter, but its rest proportions differ enough that binding it onto the police skeleton would deform the mesh. `tools/modeling/integrate_plain_walter.py` therefore preserves and exports its own rig as `assets/models/walter_plain.glb`, caps the embedded texture at 2K, and provides the same semantic animation contract. `walter_model.gd` switches the two complete rendered roots while `main.gd` drives both animation players in sync. Collision, camera, movement, evidence, saves, and `CaseState.coat` remain singular and unchanged.

`tests/walter_model_flow.gd` now requires both independently rendered meshes and rigs, verifies complete-root visibility switching in both directions, and checks the shared Walk/Surprise/Examine contract on both animation players. A recolored duplicate of the police mesh can no longer satisfy the runtime test.
