...
**Current-language addendum — September 12, 2026:** The live corpus now contains 41 concrete NPC definitions and 331 nonempty topics. `VOICE:` carries validated instrumental cue IDs; omitted NPC cues warn and receive a neutral fallback. Multiple eligible `default` topics may use positive relative `WEIGHT:` values, with immediate-repeat avoidance; unpriced defaults cost zero minutes while other unpriced topics retain the three-minute fallback. Consequential fork branches may use `OUTCOME: decision_id = value_id`; the chosen value commits only after complete playback, is immutable and saved, keeps its topic available, and limits later replays to the committed branch. Existing forks without outcomes remain fully replayable. `docs/DIALOGUE_AUTHORING.md` is the canonical complete syntax reference.
