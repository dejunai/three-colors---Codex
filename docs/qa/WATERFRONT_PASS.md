...
Dejunai requested the waterfront as a playable district, with three or four additional NPCs to be authored later. Codex added a separate outdoor hub using the existing travel, clock, save, and dialogue systems. No Web export or publication was performed; the phone experiment remains separate.
...
Existing residents retain their dialogue and IDs:
...
Use `LOCATION: waterfront` with an explicit schedule. Available, unoccupied frontage slots: `waterfront_chandlery`, `waterfront_net_loft`, `waterfront_fish_stores`, `waterfront_moorings`. For example:
...
Do not give multiple residents the same slot in the same phase. `quay`, `quay_repair`, and `quay_office` already have residents. Consult `docs/DIALOGUE_AUTHORING.md` for gates and dialogue syntax. The NPC HTML reference was regenerated from the changed location/schedule metadata.

## Validation

`tests/waterfront_flow.gd`: entry/return, travel cost, outdoor daylight setup, island scenery without route, all three residents' interaction focus, evening/night schedules, and save restoration. `tests/dialogue_catalog_flow.gd`: 29 NPCs / 135 nonempty topics, effects, timing, links, and reachable placements. `tests/town_expansion_flow.gd`: existing neighborhoods, rooms, morgue, post office, and body timeline. All passed. The older expansion test was corrected to check a placeholder resident only when its interaction actually exists; the stationer's named dialogue already replaces that placeholder.
