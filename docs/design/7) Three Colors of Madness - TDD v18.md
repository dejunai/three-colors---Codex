...
* **v17's "\~500-file library" scope language is removed. The shipped game uses 144 reusable cue IDs, not one unique recording per spoken line — a cue is shared across every line tagged with it, not generated per-line. The ElevenLabs replacement library is still being assembled and its final vocabulary may grow, but 500 files is not a current architectural requirement and shouldn't be treated as one.**  
* **Returning staff are now correctly recorded as implemented rather than hypothetical: `odell_precinct` and `coroners_assistant_morgue` exist as live scheduled NPCs for later days. One real defect surfaced while checking this, logged as a new Part Four item: the Odell precinct topics gate on `topic_done(odell, ...)` even though completed topics for that later encounter are recorded under `odell_precinct` — a naming/namespace mismatch, not a logic error in intent — so those conversations can remain repeatable when they shouldn't be. Needs a small code correction.**  
...
* **Three small refinements after this revision's initial approval, still 2026-09-12, no version bump: the ElevenLabs replacement library is not a strict drop-in for all 144 existing cue IDs (see Part Two); the cue manifest validates names during dialogue loading and automated content validation, not "at author time" (see Part Three); and a short current-state note on the shared dialogue/examination UI frame is added to Part Three.**
...
**Confirmed this pass: wordless instrumental voice cues are implemented across every authored NPC line. 403 NPC lines currently carry a voice cue, drawn from a 144-entry cue manifest (a cue is reused across every line tagged with it — this is not one recording per spoken line). See Part Three for the confirmed architecture. Walter, narration, beats, objects, notebook entries, and system text are intentionally silent by design, not by omission. The cues currently in use are the sub-par/regressed batch gathered from Grok (see `voice_pack_brief_grok.md`'s status log); a higher-quality replacement library is being assembled separately through ElevenLabs. Refined this pass: that library is not a strict drop-in for all 144 existing cue IDs. Its planned matrix is roughly 120 files using a broader delivery-style vocabulary at one take per length, against the placeholder manifest's twelve styles, three lengths, and two takes — a different shape, not a superset. It can replace matching cue IDs directly; exploiting its expanded delivery styles will require a manifest update and selective `VOICE:` reassignment in the `.dialogue` source, not a silent swap. A compatibility mapping between the two vocabularies is planned but not yet built.**
...
**Confirmed this pass: returning staff for later days are implemented, not hypothetical. `odell_precinct` and `coroners_assistant_morgue` exist as live scheduled NPCs, giving both figures a presence beyond their initial estate encounter. New defect found while confirming this: the Odell precinct topics gate on `topic_done(odell, ...)` — the original estate-scene actor ID — even though completed topics for the later precinct encounter are actually recorded under the `odell_precinct` actor ID. Because the gate checks the wrong namespace, those precinct conversations can remain repeatable when they're meant to resolve once. This is a small, well-understood fix (point the gate at `odell_precinct`'s own completion state, or record precinct topics under `odell` consistently) — logged as an open item in Part Four rather than fixed in this document.**
...
**New this pass: Odell-precinct topic-gate namespace defect. `odell_precinct`'s topics gate on `topic_done(odell, ...)` instead of `topic_done(odell_precinct, ...)`, so completed precinct conversations can remain repeatable. Small, well-understood fix; not yet applied.**
...
**Voice-cue library replacement pipeline — clarified this pass, no longer speculative scope. The present game uses 144 reusable cue IDs, not one recording per line, so the earlier "\~500-file library" framing (v17) overstated the architectural requirement and is removed. The ElevenLabs replacement library is still being assembled; its planned \~120-file matrix uses a broader delivery-style vocabulary at one take per length rather than matching the placeholder manifest's twelve-styles/three-lengths/two-takes shape. It can replace matching cue IDs directly, but exploiting its expanded styles will require a manifest update and selective `VOICE:` reassignment rather than a silent drop-in — a compatibility mapping between the two vocabularies is planned but not yet built.**
...
3. **Fix the `odell_precinct` topic-gate namespace defect (Part Three/Four) so precinct conversations resolve once rather than remaining repeatable.**  
4. **Migrate `mrs_ashcroft.dialogue`, `mrs_whitlock.dialogue`, and `mr_wick.dialogue` to the documented `morning/midday/evening/night` schedule vocabulary — still accurate and unchanged: these three files continue to use `dawn` and placeholder destinations. A consistency cleanup for the catalog-resident system specifically, with no night-placement consequence either way (Part Three).**  
5. **Confirm or rule out the `$84,000` rendering item live.**  
6. **Build the compatibility mapping between the placeholder Grok manifest and the planned ElevenLabs vocabulary (Part Two/Four), then confirm which cue IDs are direct swaps versus which need a manifest update and `VOICE:` reassignment before the ElevenLabs batch actually lands.**
