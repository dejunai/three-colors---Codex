...
5. **Build the compatibility mapping between the placeholder Grok manifest and the planned ElevenLabs vocabulary (Part Two/Four), then confirm which cue IDs are direct swaps versus which need a manifest update and `VOICE:` reassignment before the ElevenLabs batch actually lands.**
