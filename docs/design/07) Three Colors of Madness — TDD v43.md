...
**OPEN, not yet fixed: Walter's sound design has drifted from the Bible's stated rule. Bible line 417 specifies a near-total silence broken only by one impossible cough motif, until an ontological crack — the first time Walter hears glass shatter — breaks that silence open for good. `01` and `04` (both pre-existing) instead give him an ambient soundscape throughout — carriage wheels, cutlery, plaster, his own breath — with no glass-shatter beat anywhere in either file. `08` had the same gap and now has a glass-shatter scene added (Section V, the water glass at the washstand). `01` and `04` still don't have one, and bringing them fully in line would mean either adding an equivalent beat to each or walking back some of their existing ambient-sound prose — both real surgery on two long, otherwise-finished pieces, left for the author's call rather than done unilaterally.**
