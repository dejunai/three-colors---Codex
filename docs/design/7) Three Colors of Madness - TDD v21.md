...
## **`FORK:`/`OUTCOME:` One-Time Resolution — implemented, confirmed against source and test**

**Ships in `dialogue_lang.gd`/`dialogue_runtime.gd`/`dialogue_state.gd`, covered by `tests/dialogue_lang_flow.gd`. Confirmed mechanism, more specific than v20's speculative draft:**

* **Inside a `FORK:` block, any `CHOICE:` branch may additionally contain `OUTCOME: decision_id = value_id` — both sides must be a simple identifier, or the parser raises a parse error (`"OUTCOME must use 'decision_id = value_id' with simple identifiers"`). `OUTCOME:` is only legal inside a `FORK:` choice; authored elsewhere it's a separate parse error (`"OUTCOME is only valid inside a FORK choice"`).**  
* **The chosen value is held as a *pending* outcome for the whole session while the chosen branch's remaining steps play out, and is only committed to persistent dialogue state (`dialogue_state.gd`'s `outcomes` dictionary) once that dialogue session finishes. `set_outcome()` refuses to overwrite an existing key — once a `decision_id` has a recorded value, it is immutable for the remainder of the playthrough, exactly as v20 intended.**  
* **Auto-retirement, confirmed exactly as v20 specified, with the precise mechanism: every topic's `outcome_keys` are collected at parse time (walking into any `FORK:` blocks it contains). Before a topic is ever offered, the runtime checks whether any of its `outcome_keys` already has a recorded outcome; if so, the topic is excluded from that NPC's menu entirely. This is a stronger and simpler guarantee than "the specific fork is skipped" — the whole topic containing a resolved `OUTCOME:` fork retires, not just the fork prompt within it.**  
* **Later dialogue elsewhere reads the concrete choice through two gate functions: `outcome(decision_id)` (has this decision been made at all) and `outcome_is(decision_id, value_id)` (was it made this specific way) — both usable in ordinary `GATE:` expressions like any other function.**  
* **Forks that use only `CHOICE:` (no `OUTCOME:`) are completely unaffected and keep their current, repeatable behavior — this stays an opt-in, per-choice authoring decision, not a blanket change to all forks, exactly as v20 intended.**

**This closes the "specified, not yet implemented" status this document's v20 gave the feature, and definitively resolves the former Part Four item "Repeatable forks where permanent exclusivity may eventually be desired" — that mechanism now exists, is tested, and is in the author's hands to use wherever a fork's outcome should stick.**

## **Unprotected Forks — resolved, closed**

**A live dialogue-review pass, 2026-09-12, found eight `TOPIC:` blocks whose `FORK:` had neither of the two protections above: no `NOT topic_done(npc, topic_id)` in the `GATE:`, and no `OUTCOME:` on any `CHOICE:` branch (so the auto-retirement described above never engaged either). Affected: `post_office_clerk.dialogue` (`ophion_club_mail`), `county_clerk.dialogue` (`wage_claim_inquiry`), `gazette_editor.dialogue` (`press_the_arithmetic`), `local_historian.dialogue` (`dr_fenn_library`), `mr_vale.dialogue`/`quay_bookkeeper` (`missing_cart`), `mr_whitehouse.dialogue` (`name_the_dead`), `school_parent.dialogue` (`the_estate_talk`), and `widow_kessler.dialogue` (`butcher_parcels`). Each let the player replay the fork indefinitely and collect every `CHOICE` branch's evidence across separate visits — the opposite of the "mutually exclusive" intent `dialogue_lang.gd`'s own authoring comments state for `FORK:` blocks. Fixed uniformly by adding `NOT topic_done(npc, topic_id)` to each `GATE:`, the same pattern already used correctly by `quay_docker.dialogue`'s `estate_freight_carts` and `schoolteacher.dialogue`'s `the_slain_boy`. All eight topics now retire permanently after either branch is taken.**
...
**Fork revisit-with-recap pattern — open, deferred by the author, 2026-09-12. Raised while fixing the eight unprotected forks above: `NOT topic_done(npc, topic_id)` retires the whole topic permanently, so a player who picked `CHOICE` A can never revisit that NPC to be reminded what A said, let alone see that B was ever an option. The author would prefer the topic stay reachable after the fork resolves, but showing only the branch actually chosen — not the fork menu again, and not the road not taken. The mechanism for this already exists but isn't wired up this way anywhere yet: give each `CHOICE` its own `OUTCOME: decision_id = value_id`; gate the original topic on `NOT outcome(decision_id)` instead of `NOT topic_done(...)` (so it disappears once any branch is chosen, exactly once, rather than being permanently done); then author one small companion "recap" topic per branch, each gated on `outcome_is(decision_id, that_value)`, `TIME: 0`, replaying only that branch's lines with no `NOTEBOOK:`/`EVIDENCE:` lines (already collected once, on the original pass). Net cost is roughly triple the topic count per fork (one original plus one recap per branch), and a new `decision_id` per fork, so this needs deliberate author sign-off per NPC rather than a blanket sweep — not applied to the eight topics above or anywhere else pending that. See Part Six.**
...
8. **Revisit the fork revisit-with-recap pattern (Part Four) at the author's discretion, and decide whether to apply it to the eight forks just given plain `NOT topic_done(...)` protection (Part Three) or leave them permanently one-shot as-is.**
