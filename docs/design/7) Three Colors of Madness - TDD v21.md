...
* **Multiple `TOPIC: default` blocks are allowed. Without any `WEIGHT:` authored, the first eligible, nonempty default wins deterministically, preserving existing specific-before-fallback gate cascades exactly as before this feature existed. As soon as *any* eligible default carries a `WEIGHT:`, all eligible defaults form a weighted pool (an eligible default with no `WEIGHT:` counts as weight 1); one is chosen by weighted random draw.**  
...
6. **Confirm or rule out the `$84,000` rendering item live.**  
