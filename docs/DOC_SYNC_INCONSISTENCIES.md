...
## 2026-09-23 pass (fourth): post-`c12fc7d` refresh, a real regression found and fixed, and a partially-applied fix caught

Scope: sync README.md/ARCHITECTURE.md/PORTAL_AUTHORING.md/run_all_qa.py/.gitignore against everything committed through `c12fc7d`/`45b54b9` (the last commits of the day, both after the "(third)" pass above was written). Design Bible and TDD not edited.

- **The "(third)" pass's "RESOLVED" claim for the portal `TIME:` doc drift was only half true.** Direct read of `docs/PORTAL_AUTHORING.md` found its "Completion, consequences, evidence, and time" section (a *second*, separate paragraph from the one the third pass fixed) still said "`TIME` defaults to 3 minutes when omitted... the charge fires on every completed traversal." Fixed now. Lesson repeated from this file's own established practice: a claim that a fix "landed" needs the actual file re-grepped for every place the stale text appeared, not just the one place that was checked.
- **Real, reproducible regression found and fixed, not just a doc gap:** `tests/dialogue_catalog_flow.gd` crashed (`Assertion failed: Actor cannot be focused: morgue_coroner got morgue_tables`) because the same-day "Autumn of '18" pass (`d6aafa2`) placed the new `morgue_tables` object hotspot within interaction range of the pre-existing `morgue_coroner`/`coroners_assistant_morgue` staff work station. This was **not caught by either of the same-day passes that came after it** (`120a488`'s "Documents refresh and bump" and the "(third)" DOC_SYNC pass both post-date `d6aafa2` and both claimed clean NPC/topic counts without this test actually completing). Fixed by moving `morgue_tables` to a clear spot in `town_expansion.gd`; reran `dialogue_catalog_flow.gd`, `test_placement_audit.gd` (0 collisions), and the full `python tests/run_all_qa.py` aggregate — all clean. See the matching `docs/ARCHITECTURE.md` dated section for the full root-cause writeup.
- **Fresh, directly-verified counts (treat everything above this line in this file as superseded for these four numbers):** 74 `.dialogue` files / 73 concrete NPCs (direct file count) / 468 nonempty topics / 484 raw `TOPIC:` headers (`tests/dialogue_catalog_flow.gd`, `Select-String`); 745 NPC lines (`tests/instrument_voice_flow.gd`); **39 suites** in `tests/run_all_qa.py` (counted the `tests = [...]` tuples directly, then ran the full aggregate clean) — not 36 or 38 as logged earlier the same day.
- **`objects/town.object` grew to 4 ids** (added `morgue_tables`); `README.md`/`docs/ARCHITECTURE.md` said 3.
- **`.gitignore` gap:** `da0ac9f` ("sqlite modified") committed changes to a local Wrangler dev-database WAL/SHM file under `cloudflare/three-colors-worker/.wrangler/` — this directory was never in `.gitignore`. Added `cloudflare/three-colors-worker/.wrangler/` to `.gitignore` (does not retroactively untrack already-committed copies; a `git rm -r --cached` would be a separate, larger decision left to the author).
- **Fixed:** `README.md` (dialogue/object counts above, coroner's-assistant-vs-morgue-coroner model split, `cast_model.gd` facing/gender wording, portal `TIME:` semantics, suite count), `docs/ARCHITECTURE.md` (object-id count, new dated section), `docs/PORTAL_AUTHORING.md` (the missed second paragraph), `.gitignore`, `tests/dialogue_catalog_flow.gd`'s underlying cause (`town_expansion.gd`).
- **Not evaluated this pass:** the Design Bible v18 canon items TDD v46 Part Four tracks (unnamed Observers, "Mason" wording, the coroner's-assistant pronoun, etc.) — those are TDD-owned per its own Part Six, not re-audited here.
