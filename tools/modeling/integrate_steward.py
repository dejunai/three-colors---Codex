"""Integrate Meshy_Steward-Animations.glb into Godot.

Runs in Blender 5.2 headless:
  & 'C:\\Program Files\\Blender Foundation\\Blender 5.2\\blender.exe' --background --python tools/modeling/integrate_steward.py
"""
...
OUT_GLB = MODEL_DIR / "steward.glb"

SRC_GLB = ROOT / "archive" / "Meshy_Steward-Animations.glb"
if not SRC_GLB.exists():
    SRC_GLB = ROOT / "Meshy_Steward-Animations.glb"
...
def main() -> None:
    ...
    armature.name = "StewardSkeleton"
    armature.data.name = "StewardSkeleton"
    mesh_obj.name = "StewardMesh"
    mesh_obj.data.name = "StewardMesh"

    # Build root Steward node
    root = bpy.data.objects.new("Steward", None)
    ...
    # 5 animations (2 idle, chat, listen, custom cleaning glass)
    # =========================================================================
    # 1. Clean_Glass (custom glass cleaning idle): 01a0c52e-c79d-71fb-b49b-e363e7cd0dd9 (121 frames)
    clean_act = bpy.data.actions.get("01a0c52e-c79d-71fb-b49b-e363e7cd0dd9")
    if clean_act:
        act_clean = clean_act.copy()
        act_clean.name = "Clean_Glass"
        act_clean.frame_start = 1
        act_clean.frame_end = 121
        act_clean.use_fake_user = True

    # 2. Idle (calm standing posture): Idle_3 (240 frames)
    ...
    # 3. Idle_Alt (slight weight adjustment): Idle_11 (47 frames)
    ...
    # 4. Confer (conversational discussion): Stand_and_Chat (125 frames)
    ...
    # 5. Listen (attentive listening gestures): Listening_Gesture (225 frames)
    ...
    target_actions = ["Clean_Glass", "Idle", "Idle_Alt", "Confer", "Listen"]
    ...
    # Set Clean_Glass as active action at frame 0 (primary atmospheric idle)
    if bpy.data.actions.get("Clean_Glass"):
        armature.animation_data.action = bpy.data.actions["Clean_Glass"]
    ...
    print(f"Steward triangle count: {tri_count}")
