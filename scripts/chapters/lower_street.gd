...
const LOWER_DWELLING_MODEL = preload("res://assets/models/exteriors/pickman_house_2.glb")

func block(g: Node, p: Vector3, size: Vector3, tint: String, solid: bool = false, kind: String = "cracked_plaster", parent: Node3D = null) -> MeshInstance3D:
...
func build(g: Node) -> void:
	...
	var legacy_dwelling := Node3D.new()
	legacy_dwelling.name = "LegacyLowerDwellingCollisionVisuals"
	g.add_child(legacy_dwelling)
	var rendered_landmarks := Node3D.new()
	rendered_landmarks.name = "RenderedLowerLandmarks"
	...
	for i in 6:
		...
		var facade_parent: Node3D = legacy_dwelling if i == 1 else g
		var width = 12.0
		var height = 5.2
		...
		block(g, p + Vector3(0, height / 2, back * 4), Vector3(width, height, 7), wall_tint, true, wall_kind, facade_parent)
		block(g, p + Vector3(0, height + 0.2, back * 4), Vector3(width + 0.6, 0.4, 7.5), "303a34", false, "slate", facade_parent)
		for x in [-width * 0.32, width * 0.32]:
			block(g, p + Vector3(x, 2.1, back * 0.4), Vector3(2, 1.8, 0.14), "27372f", false, "wood", facade_parent)
			for dx in [-1.08, 0, 1.08]:
				block(g, p + Vector3(x + dx, 2.1, back * 0.25), Vector3(0.09, 2, 0.12), "8c907b", false, "wood", facade_parent)
		...
		block(g, p + Vector3(-3, 0.45, -back * 0.7), Vector3(2.4, 0.18, 0.65), "5d654f", true, "wood", facade_parent)
		for y in [0.6, 1.2, 1.8, 2.4, 3.0, 4.1]:
			block(g, p + Vector3(0, y, back * 0.3), Vector3(width, 0.035, 0.07), "493f35", false, "wood", facade_parent)
		if spec[3] == "exterior":
			for y in [0.7, 1.5, 2.3]:
				block(g, p + Vector3(0, y, 0), Vector3(1.8, 0.2, 0.1), "625d4c", false, "wood", facade_parent)
		else:
	...
	var rendered_dwelling := LOWER_DWELLING_MODEL.instantiate() as Node3D
	rendered_dwelling.name = "DwellingTwoExterior"
	rendered_dwelling.position = Places.front(1) + Vector3(0, 0, -4)
	rendered_dwelling.scale = Vector3(6.85, 2.6, 3.55)
	rendered_landmarks.add_child(rendered_dwelling)
	legacy_dwelling.visible = false
