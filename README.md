...
`python tests/run_all_qa.py` is the maintained aggregate runner. Its current 28-suite set covers the dialogue, object and portal grammars/templates/live content; opening, town, service-passage loop, usability integration, and the complete steward-to-glass ending path; and the accepted character, victim, exterior, texture, pickup, Pickman-frontage, and lower-district model checks. `tests/dialogue_catalog_flow.gd`, the population/placement checks, pocket-watch, telemetry, camera and presentation checks remain focused suites outside that aggregate and should be run when their areas change. The `.cmd` launchers remain convenient partial checks, not the authoritative complete set.
