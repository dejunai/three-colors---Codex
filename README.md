...
`python tests/run_all_qa.py` is the maintained aggregate runner. Its current 12-suite set covers the dialogue, object and portal grammars/templates/live content plus opening, town, service-passage loop and usability integration; the latest checked-in run is clean in `qa_run_log.txt`. `tests/break_flow.gd`, `tests/dialogue_catalog_flow.gd`, the population/placement checks, pocket-watch, telemetry, camera and presentation checks remain focused suites outside that aggregate and should be run when their areas change. The `.cmd` launchers remain convenient partial checks, not the authoritative complete set.
